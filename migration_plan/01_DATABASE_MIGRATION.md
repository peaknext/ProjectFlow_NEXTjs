...
- Phases - ระยะของโปรเจค (ใช้โดย createPhase(), createPhasesBatch())
...
2. **Phase** - ระยะของโปรเจค
3. **HospitalMission** - พันธกิจโรงพยาบาล
4. **ITGoal** - เป้าหมาย IT ที่เชื่อมโยงกับพันธกิจ
5. **ActionPlan** - แผนปฏิบัติการที่เชื่อมโยงกับเป้าหมาย IT และโปรเจค
...
function exportUsers() {
  ...
  return data.slice(1).map((row, index) => ({
    ...
    isVerified: row[SCHEMA.USERS.IS_VERIFIED - 1] === true || row[SCHEMA.USERS.IS_VERIFIED - 1] === 'TRUE',
...
function exportTasks() {
  ...
  return data.slice(1).map((row, index) => ({
    ...
    isClosed: row[SCHEMA.TASKS.IS_CLOSED - 1] === true || row[SCHEMA.TASKS.IS_CLOSED - 1] === 'TRUE',
...
function exportChecklistItems() {
  ...
  return data.slice(1).map((row, index) => ({
    ...
    isChecked: row[SCHEMA.CHECKLIST_ITEMS.IS_CHECKED - 1] === true || row[SCHEMA.CHECKLIST_ITEMS.IS_CHECKED - 1] === 'TRUE',
...
async function main() {
  ...
  try {
    await prisma.$transaction(async (tx) => {
      ...
      for (const proj of data.projects) {
        const created = await tx.project.create({
          data: {
            ...
            createdAt: proj.dateCreated ? new Date(proj.dateCreated) : new Date(),
          },
        });
      ...
      for (const task of data.tasks) {
        const created = await tx.task.create({
          data: {
            ...
            assigneeUserId: task.assigneeUserId ? idMaps.users.get(task.assigneeUserId) : null,
            statusId: idMaps.statuses.get(task.statusId)!,
            priority: task.priority,
            startDate: task.startDate ? new Date(task.startDate) : null,
            dueDate: task.dueDate ? new Date(task.dueDate) : null,
            parentTaskId: task.parentTaskId ? idMaps.tasks.get(task.parentTaskId) : null,
            ...
            userClosedId: task.userClosedId ? idMaps.users.get(task.userClosedId) : null,
      ...
      console.log(`✅ Migrated ${data.hospitalMissions?.length || 0} hospital missions\n`);
      ...
      for (const goal of data.itGoals || []) {
        const created = await tx.iTGoal.create({
          data: {
            name: goal.name,
            hospitalMissionId: idMaps.hospitalMissions.get(goal.hospitalMissionId)!,
      ...
      console.log(`✅ Migrated ${data.actionPlans?.length || 0} action plans\n`);
      ...
      for (const phase of data.phases || []) {
        const created = await tx.phase.create({
          data: {
            ...
            createdAt: phase.dateCreated ? new Date(phase.dateCreated) : new Date(),
          },
        });
      ...
      console.log(`✅ Migrated ${data.checklistItems?.length || 0} checklist items\n`);
...
// Helper functions
function mapUserRole(role: string): 'ADMIN' | 'CHIEF' | 'LEADER' | 'HEAD' | 'MEMBER' | 'USER' {
...
function mapProjectStatus(status: string): 'ACTIVE' | 'COMPLETED' | 'ON_HOLD' | 'ARCHIVED' {
  const map: Record<string, any> = {
    'Active': 'ACTIVE',
    'Completed': 'COMPLETED',
    'On Hold': 'ON_HOLD',
...
async function preMigrationCheck(data: any) {
  const checks = {
    ...
    orphanedTasks: data.tasks.filter((t: any) =>
      !data.projects.find((p: any) => p.originalId === t.projectId)
    ).length,

    orphanedStatuses: data.statuses.filter((s: any) =>
      !data.projects.find((p: any) => p.originalId === s.projectId)
    ).length,

    // Check for invalid foreign keys
    invalidAssignees: data.tasks.filter((t: any) =>
      t.assigneeUserId && !data.users.find((u: any) => u.email === t.assigneeUserId)
    ).length,
...
async function verifyMigration(originalData: any) {
  ...
  const randomTask = originalData.tasks[Math.floor(Math.random() * originalData.tasks.length)];
  const migratedTask = await prisma.task.findFirst({
    where: { name: randomTask.name, createdAt: new Date(randomTask.dateCreated) },
  });
  checks.push({
